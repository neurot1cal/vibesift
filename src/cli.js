...
Commands
  ...
  vibesift ship  <slug> diff <url>   (manually set the ship diff URL — e.g. a merged PR link)
...
function cmdShip(argv) {
  const slug = argv[0];
  const sub = argv[1];
  if (!slug) die('usage: vibesift ship <slug> <task|diff> ...');
  if (sub === 'task') {
    const action = argv[2];
    const { state, path } = loadState(slug);
    if (action === 'add') {
    ...
    const url = argv[2];
    if (!url) die('usage: vibesift ship <slug> diff <url>');
    const { state, path } = loadState(slug);
    setDiffUrl(state, url);
    const r = saveAndCommit(state, path, `vibesift: ship diff URL set on ${slug}`);
    reportCommit(r, `diff URL set on ${slug}`);
  } else {
    die('usage: vibesift ship <slug> <task|diff> ...');
  }
}
